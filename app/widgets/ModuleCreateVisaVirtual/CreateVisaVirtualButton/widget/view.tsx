...
export function CreateVisaVirtualButtonView({
  createVisaVirtualIsFetching,
  createCard
}: IOwnProps) {
  return (
    <TouchableOpacity disabled={createVisaVirtualIsFetching} onPress={createCard} activeOpacity={TOUCHABLE_OPACITY} style={styles.button}>
      <LinearGradient style={styles.buttonInner} start={{x: 0, y: 0}} end={{x: 1, y: 0}} colors={GRADIENT_COLORS}>
        {
          createVisaVirtualIsFetching ?
            ...
            <Text style={styles.buttonTitle}>Создать</Text>
        }
      </LinearGradient>
    </TouchableOpacity>
  )
