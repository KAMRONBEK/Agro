...
export function MainSelectCurrencyView({

}: IOwnProps) {
  function renderRow(option: any, index: number, isSelected: boolean) {
    return (
      <TouchableOpacity key={index} style={[styles.currencyBox, Number(index) === OPTIONS.length - 1 && styles.currencyLastBox, Number(index) !== 0 && styles.currencyBoxBottomMargin]}>
        ...
        <View style={styles.currencyRightBox}>
          <Text style={styles.rate}><Text style={styles.rateBold}>9500.00</Text> сум</Text>
        </View>
      </TouchableOpacity>
    )
  }
