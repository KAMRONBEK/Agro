...
export function MainTransactionsHistoryView({
  ...
  function renderEmptyList() {
    return (
      <View style={styles.emptyListBox}>
        <Text style={styles.emptyListTitle}>Список транзакций пуст</Text>
      </View>
    )
  }
  ...
  function renderTransactionByPlatform(item: ITransaction, index: number) {
    switch(Platform.OS) {
      case 'android': return (
        <TouchableOpacityGH onPress={details(item.id)} activeOpacity={TOUCHABLE_OPACITY} style={[styles.button, index !== 0 && styles.buttonBorder]}>
      ...
      case 'ios': return (
        <TouchableOpacity onPress={details(item.id)} activeOpacity={TOUCHABLE_OPACITY} style={[styles.button, index !== 0 && styles.buttonBorder]}>
          <View style={styles.buttonLogoBox}>
            <Image style={styles.buttonLogo} resizeMode="contain" source={{uri: item.logo}} />
          </View>
          <View style={styles.serviceNameBox}>
  ...
  function renderContent() {
    return (
      <>
        ...
        {
          transactions.length && !transactionsIsFetching ? transactions.map(renderTransaction) : renderEmptyList()
        }
      </>
    )
  }
