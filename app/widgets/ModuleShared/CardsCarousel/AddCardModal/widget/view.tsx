      ...
      <>
        <View style={styles.content}>
          <View style={styles.header}>
            <Text style={styles.headerLabel}>Добавить карту</Text>
            ...
            <View style={styles.cardView}>
              <View style={styles.inputBox}>
                ...
                <TextInputMask
                  ...
                  style={[styles.input, styles.numberInput, cardNumberField.errors.length && styles.inputBoxError]}
                  placeholder={'Номер карты (16-значный)'}
                  placeholderTextColor={Palette.grey}
                  underlineColorAndroid={Palette.transparent}
                  mask='[0000] [0000] [0000] [0000]'
                />
                <View style={styles.errorBox}>
                  {cardNumberField.errors.map(renderError)}
                </View>
              </View>
              <View style={styles.inputBox}>
                <Text style={styles.inputLabel}>Срок действия карты</Text>
                <TextInputMask
                  onChangeText={onChangeExpireDate}
                  keyboardType='number-pad'
                  editable={!createCardIsFetching}
                  value={expireDateField.value}
                  style={[styles.input, styles.expireInput, expireDateField.errors.length && styles.inputBoxError]}
                  placeholder={'00/00'}
                  placeholderTextColor={Palette.grey}
                  underlineColorAndroid={Palette.transparent}
                  mask='[00]/[00]'
                />
                <View style={styles.errorBox}>
                  {expireDateField.errors.map(renderError)}
                </View>
              </View>
            </View>
          </View>
          <TouchableOpacity onPress={createCard} activeOpacity={TOUCHABLE_OPACITY} style={styles.addCardButton}>
            {
              createCardIsFetching ? 
                <ActivityIndicator size='small' color={Palette.white} animating={createCardIsFetching} /> :
                <Text style={styles.addCardButtonText}>Готово</Text>
            }
          </TouchableOpacity>
        </View>

        <VerifyCardModal 
          isVisible={verifyCardModalIsVisible}
          closeModal={closeVerifyCardModal}
        />
      </>
    </TemplateModal>
  )
