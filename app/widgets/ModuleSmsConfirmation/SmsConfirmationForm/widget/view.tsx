...
export function SmsConfirmationFormView({ field, phoneField, onChange }: IOwnProps) {
	...
	return (
		<View style={styles.container}>
			<View style={styles.phoneBox}>
				...
				/>
				{!!phoneField.errors.length && (
					<View style={styles.phoneErrorBox}>{phoneField.errors.map(renderError)}</View>
				)}
			</View>
			...
			<View style={styles.form}>
				<CodeInput
					...
					onFulfill={onChange}
				/>
				{!!field.errors.length && <View style={styles.errorBox}>{field.errors.map(renderError)}</View>}
			</View>
